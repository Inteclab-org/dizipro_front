          ...
          <nav className="items-center gap-[42px] leading-[24px] flex flex-col pt-[150px] pb-[70px] grow">
            {
              menuItems.map((menuItem: MenuItem, index) => (
                !menuItem.link ? (
                  <DialogTrigger key={`menu-item-${index + 1}`} className="text-[30px] tracking-[-0.84px] leading-[1.1] font-bold uppercase text-black/70 text-center sm:text-[38px]">
                    {menuItem.name}
                  </DialogTrigger>
                ) : (
                  <Link key={`menu-item-${index + 1}`} href={menuItem.link} className="text-[30px] tracking-[-0.84px] leading-[1.1] font-bold uppercase text-black/70 text-center sm:text-[38px]">
                    {menuItem.name}
                  </Link>
                )
              ))
